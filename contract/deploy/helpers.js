...
function loadEnv() {
  ...
  if (!(CHAIN_NAME && RPC_ENDPOINT && DEPLOYER_PRIVATE_KEY)) {
    throw new Error(
      "Must populate all values in .env - see .env.example for full list"
    );
  }
...
function getDeployedAddresses(CHAIN_NAME) {
  const filename = `./deploy/deployed/${CHAIN_NAME}.json`;
...
async function verifyContract(address, constructorArguments) {
  ...
  if (!CHAIN_NAME) {
    ...
    throw new Error(
      `CHAIN_NAME in .env file is "${CHAIN_NAME}" but hardhat --network in package.json is "${hre.network.name}; change them to match"`
    );
  }
  try {
